(function ($) {
  ...
  var bgPickerPreview = function (bgPickerArea, data) {
    $(bgPickerArea)
      ...
      .css({ "background-color": data.color ? data.color : "transparent" });
  };
  ...
  var removeFormInputErrorHighlight = function () {
    $("[class^=form_field_]").on("input change", function () {
      $(this)
        .closest(".form_field_with_errors")
        .removeClass("form_field_with_errors");
    });
  };

  var setImageOrientation = function ($contentItemBox, width, height) {
    var $imgDropAreaTarget = $contentItemBox.find(
        ".js-content-item-img-drop-area"
      ),
    ...
    if ($imgDropAreaTarget.hasClass("image-square")) {
      $cropToggleButton.removeClass("is-visible").addClass("is-hidden");
    } else {
      $cropToggleButton.removeClass("is-hidden").addClass("is-visible");
    }
  };

  var setItemImage = function (
    $contentItemBox,
    $imgDropArea,
    itemId,
    imageId,
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $contentItemBox
          .removeClass("not-loaded with-error")
          .addClass("is-loaded");
        $contentItemBox
          .find(".edy-img-drop-area-placeholder")
          .css("opacity", 1);
      ...
      error: function (data) {
        $contentItemBox
          .removeClass("not-loaded is-loaded with-error")
          .addClass("with-error");
      },
    });
  };
  ...
  var bindContentItemImgDropAreas = function (placeholderText) {
    $(".js-content-item-img-drop-area").each(function (
      index,
      imgDropAreaTarget
    ) {
      ...
      var imgDropArea = new Edicy.ImgDropArea($imgDropAreaTarget, {
        ...
        placeholder:
          ...
          "</div>",
        removeBtn:
          '<div class="edy-img-drop-area-remove-image">' +
  ...
  var bindContentItemImageCropToggle = function () {
    $(".js-toggle-crop-state").on("click", function () {
      var $contentItemBox = $(this).closest(".js-content-item-box"),
        $imgDropAreaTarget = $contentItemBox.find(
          ".js-content-item-img-drop-area"
        ),
        itemId = $contentItemBox.data("item-id"),
        itemType = $contentItemBox.data("item-type"),
      ...
      if ($imgDropAreaTarget.hasClass("is-cropped")) {
        $imgDropAreaTarget.removeClass("is-cropped").addClass("not-cropped");

        imageCropState = "not-cropped";
      } else {
        $imgDropAreaTarget.removeClass("not-cropped").addClass("is-cropped");
  ...
  var handleProductPageContent = function () {
    ...
    var changeProductImagePos = function () {
      ...
      if ($(".js-buy-btn-content .edy-buy-button-container").length >= 1) {
        if ($(window).width() < 640) {
          if (
            $(".js-product-right-content + .js-product-gallery").length === 0
          ) {
  ...
  var handleProductImageClick = function (product_id) {
    if (editmode()) {
      $(".product-content .product-image").click(function () {
        window
          .open("/admin/ecommerce/products/" + product_id, "_blank")
          .focus();
      });
    }
  };
  ...
  var bindRootItemSettings = function (rootItemValuesObj) {
    ...
    $(".js-root-item-settings-toggle").each(function (
      index,
      languageMenuSettingsButton
    ) {
      var rootItemSettingsEditor = new Edicy.SettingsEditor(
        languageMenuSettingsButton,
        {
          ...
          containerClass: [
            "js-root-item-settings-popover",
            "js-prevent-sideclick",
          ],
